...
export default function Hero() {
  ...
  return (
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0a1628]">
      ...
      {/* The Foundation's seal, sunk into the background.
          A photograph was tried here and dropped: the only frames available are
          indoor event shots, and at an opacity safe for white type they read as
          murk rather than as an image. The seal is the Foundation's own mark and
          needs no explaining, so it can sit at 6% and still register.

          The source is 256px square (LogoMark explains why it is kept small)
          and this renders at up to 760, so it is enlarged — but at 6% over navy
          that is invisible, and the alternative is shipping a second, heavier
          copy of the seal for something nobody is meant to look at directly.
          No `priority`: the navbar already preloads this exact file, so it is
          in cache before this ever paints. */}
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
        <Image
          ...
          width={760}
          height={760}
          className="h-auto w-[min(86vw,760px)] max-w-none opacity-[0.06]"
        />
      </div>
